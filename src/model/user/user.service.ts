...
import { findUserById, findUserByUsername, insertUser, updateImageById, updateUserById } from './user.repository';
...
export const uploadImageById = async (id: string, image: string) => {
	const findUser = await getUserById(id);
	if (!findUser) throw Error('Username does not exist');

	const user = await updateImageById(id, image);
	return user;
};
